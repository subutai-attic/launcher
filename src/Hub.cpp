...
namespace SubutaiLauncher {
    ...
    size_t Hub::handleHeaders(char* data, size_t size, size_t nitems, void *p) {
        return static_cast<Hub*>(p)->handleHeadersImpl(data, size, nitems);
    }

    size_t Hub::handleHeadersImpl(char* data, size_t size, size_t nitems) {
        if (size * nitems <= 2) {
            return nitems * size;
        }
