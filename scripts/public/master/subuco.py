...
class SubutaiPeer:
    ...
    def InstallUbuntu(self):
        ...
        td = "/var/snap/subutai-master/common/lxc/tmpdir/"
        awk = " | awk '{print $5}'"
    ...
    def InstallSnap(self):
        ...
        command = 'sudo snap install --beta --devmode subutai-master > /tmp/subutai-snap.log 2>&1'
        attempts = 0
        while attempts < 10:
            subutai.SSHRun("sudo snap install --beta --devmode subutai-master > /tmp/subutai-snap.log 2>&1")

            out = subutai.SSHRunOut("which subutai-master >/dev/null; echo $?")
            if out == '0':
                return 0
            sleep(30)
            attempts = attempts + 1
